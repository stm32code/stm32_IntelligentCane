...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"disc\":\"%d\",", device_state_init.Distance); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"waring\":\"%d\",", device_state_init.Fall_State); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"key_waring\":\"%d\"", device_state_init.Key_State); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
